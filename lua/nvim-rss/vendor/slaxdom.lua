...
-- Vendored in nvim-rss from https://github.com/Phrogz/SLAXML (MIT License)
local SLAXML = require("nvim-rss.vendor.slaxml")
function SLAXML:dom(xml, opts)
	...
	local builder = SLAXML:parser({
		...
		attribute = function(name, value, nsURI, nsPrefix)
			...
			local attr =
				{ type = "attribute", name = name, nsURI = nsURI, nsPrefix = nsPrefix, value = value, parent = rich
						and current
					or nil }
			if rich then
		...
		text = function(value, cdata)
			...
			push(
				current.kids,
				{ type = "text", name = "#text", cdata = cdata and true or nil, value = value, parent = rich and current
					or nil }
			)
		end,
