local slaxdom = require("nvim-rss.vendor.slaxdom")
		...
		for i, el in ipairs(entry_el:getChildren("*")) do
			...
			elseif (format_str == "atom" and tag == "link") or (format_str == "rss" and tag == "atom:link") then
				...
				if link.rel == "enclosure" then
					tinsert(
						entry.enclosures,
						{ href = link.href, length = el:getAttr("length"), type = el:getAttr("type") }
					)
				end
			...
			elseif format_str == "rss" and tag == "enclosure" then
				tinsert(
					entry.enclosures,
					{ url = el:getAttr("url"), length = el:getAttr("length"), type = el:getAttr("type") }
				)
			...
			-- content
			elseif
				(format_str == "atom" and tag == "content")
				or (
					format_str == "rss"
					and (tag == "body" or tag == "xhtml:body" or tag == "fullitem" or tag == "content:encoded")
				)
			then
			...
			-- published
			elseif
				(format_str == "atom" and (tag == "published" or tag == "issued"))
				or (
					format_str == "rss" and (tag == "dcterms:issued" or tag == "atom:published" or tag == "atom:issued")
				)
			then
			...
			-- updated
			elseif
				(format_str == "atom" and (tag == "updated" or tag == "modified"))
				or (format_str == "rss" and (tag == "dc:date" or tag == "pubDate" or tag == "dcterms:modified"))
			then
...
function feedparser.parse(xml_string, base_url)
	...
	local rootElement = XMLElement.new(doc.root)
